...
export class MyProfileComponent implements OnInit {
    ...
    constructor(private dialog: MatDialog,private updateprofileService:UpdateprofileService,private domSanitizer:DomSanitizer,private router:Router,private appProvider:AppProvider,private allPostsService:AllPostsService,location: Location,  private element: ElementRef) {
      ...
      this.getSavedPost();
      this.updateProfileModel.state=this.userInfo.state
      this.updateProfileModel.district=localStorage['district']
      this.updateProfileModel.dateOfBirth=localStorage['dateOfBirth']
      this.updateProfileModel.gender=localStorage['gender']
      this.updateProfileModel.block=localStorage['block']
   ...
   onSend(field){
    ...
    updateProfile(){
      this.updateprofileService.updateProfile(this.updateProfileModel,this.userId).subscribe(response=>{
        console.log(response);
        localStorage['state']=this.updateProfileModel.state;
        localStorage['district']=this.updateProfileModel.district;
        localStorage['dateOfBirth']=this.updateProfileModel.dateOfBirth;
        localStorage['gender']=this.updateProfileModel.gender;
        localStorage['block']=this.updateProfileModel.block;
      },error=>{
        console.log(error);
      })
    }
